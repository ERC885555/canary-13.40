...
local function exerciseTrainingEvent(playerId, tilePosition, weaponId, dummyId)
	...
	_G.OnExerciseTraining[playerId].event = addEvent(exerciseTrainingEvent, vocation:getBaseAttackSpeed() / (configManager.getFloat(configKeys.RATE_EXERCISE_TRAINING_SPEED) * 1000), playerId, tilePosition, weaponId, dummyId)
	return true
end
