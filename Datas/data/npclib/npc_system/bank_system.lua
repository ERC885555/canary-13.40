...
function Npc:parseBank(message, npc, creature, npcHandler)
	...
	elseif npcHandler:getTopic(playerId) == 7 then
		if MsgContains(message, "yes") then
			...
			-- Nova linha para calcular e entregar as Tibianum Coins no saque (1.000.000 gp)
			local tibianumCoins = math.floor(totalValue / 1000000)
			...
			-- Adiciona as pilhas no cálculo de peso e espaço
			local tibianumPiles = math.floor((tibianumCoins + 99) / 100)
			...
			local totalPiles = tibianumPiles + crystalPiles + platinumPiles + goldPiles
