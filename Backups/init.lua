...
function GameStore.processItemPurchase(player, offerId, offerCount, movable, setOwner)
	...
	local customRelics ={
				...
				9215, -- Distance Spitter
				20182, -- Protection Ring of Ending
				3080, -- Broken Amulet
				9218, -- Badge of Master Protection
				37335, -- Final Fate
	...
	for i = 1, #customRelics do
		if offerId == customRelics[i] then
			isCustomRelic = true
			break
	end
end

	if isCustomRelic then
		-- Se for uma das suas relíquias, injeta direto na Backpack principal (LIVRE DE TRAVAS!)
		local mainBackpack = player:getSlotItem(SLOT_BACKPACK)
		if mainBackpack then
			mainBackpack:addItem(offerId, offerCount or 1)
		else
			Game.createItem(offerId, offerCount or 1, player:getPosition())
		end
		player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "Sua relíquia especial foi entregue diretamente na sua mochila comum!")
	else
		-- Se for qualquer outro item comum do Tibia, mantém o fluxo original indo para a Store Inbox roxa
